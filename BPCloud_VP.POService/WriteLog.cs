...
namespace BPCloud_VP_POService
{
    public static class WriteLog
    {
        ...
        public static void WriteToGRGIFile(string Message)
        {
            StreamWriter sw = null;
            try
            {
                string path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "GRGILogFiles");
                if (!Directory.Exists(path))
                {
                    Directory.CreateDirectory(path);
                }
                ...
                string today = dt.ToString("yyyyMMdd");
                string Log = today + ".txt";
                if (File.Exists(AppDomain.CurrentDomain.BaseDirectory + "\\GRGILogFiles\\Log_" + yday + ".txt"))
                {
                    System.GC.Collect();
                    System.GC.WaitForPendingFinalizers();
                    File.Delete(AppDomain.CurrentDomain.BaseDirectory + "\\GRGILogFiles\\Log_" + yday + ".txt");
                }
                sw = new StreamWriter(AppDomain.CurrentDomain.BaseDirectory + "\\GRGILogFiles\\Log_" + Log, true);
                sw.WriteLine($"{DateTime.Now.ToString()} : {Message}");
                sw.Flush();
                sw.Close();
            }
        ...
        public static void WriteToGRGIFile(string ControllerAction, Exception ex)
        {
            StreamWriter sw = null;
            try
            {
